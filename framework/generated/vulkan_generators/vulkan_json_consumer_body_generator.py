...
class VulkanJsonConsumerBodyGenerator(BaseGenerator):
    ...
    def makeConsumerFuncBody(self, returnType, name, values):
        ...
        self.newline()
        self.wc('    OutputIndentJson(outputFile, 2); // TWP')
        self.wc('    OutputStringJson(outputFile, "{\\n");')
        self.wc('    OutputIndentJson(outputFile, 3); // TRP')
        ...
        if returnType != 'void':
            self.wc('    OutputIndentJson(outputFile, 3); // TKP')
            ...
            if self.isEnum(returnType):
                ...
                self.wc('    OutputAddrJson(returnValue);')
            else:
            ...
            self.wc('    OutputStringJson(outputFile, "\\",\\n");')
        self.wc('    OutputIndentJson(outputFile, 3); // TTP')
        self.wc('    OutputStringJson(outputFile, "\\"args\\" :\\n");')
        self.wc('    OutputIndentJson(outputFile, 3);')
        self.wc('    OutputStringJson(outputFile, "[\\n");')
        ...
        for value in values:
            ...
            self.wc('    OutputIndentJson(outputFile, 4); // UWP')
            self.wc('    OutputStringJson(outputFile, "{\\n");')
            OutputValue.outputValue(self, value, name, "")
            self.wc('    OutputIndentJson(outputFile, 4);')
            if value == values[-1]:
        ...
        self.wc('    OutputIndentJson(outputFile, 2); // RRW')
        if name != "vkQueuePresentKHR":
            ...
            self.wc('    frameNumber++;')
            self.wc('    OutputIndentJson(outputFile, 1); // RWW')
            self.wc('    OutputStringJson(outputFile, "]\\n");')
            self.wc('    OutputStringJson(outputFile, "},\\n");')
            self.wc('    OutputStringJson(outputFile, "{\\n");')
            self.wc('    OutputIndentJson(outputFile, 1);')
            self.wc('    OutputStringJson(outputFile, "\\"frameNumber\\" : \\"");')
            self.wc('    OutputSignedDecimalJson(outputFile, frameNumber);')
            self.wc('    OutputStringJson(outputFile, "\\",\\n");')
            self.wc('    OutputIndentJson(outputFile, 1);')
